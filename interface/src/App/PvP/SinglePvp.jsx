...
class SinglePvp extends React.PureComponent {
    ...
    onNameChange(event, name) {
        ...
        var ivSet = calculateMaximizedStats(event.value, 40.0, this.props.parentState.pokemonTable)
        //set state
        this.setState({
            [name]: {
                ...this.state[name],
                ...
                "Lvl": ivSet[this.props.parentState.league]["Default"]["Level"],
                "Atk": ivSet[this.props.parentState.league]["Default"]["Atk"],
                "Def": ivSet[this.props.parentState.league]["Default"]["Def"],
                "Sta": ivSet[this.props.parentState.league]["Default"]["Sta"],
                "AtkStage": "0",
                "DefStage": "0",
                "InitialHP": "0",
                "InitialEnergy": "0",
                ...
                effAtk: calculateEffStat(event.value, ivSet[this.props.parentState.league]["Default"]["Level"], ivSet[this.props.parentState.league]["Default"]["Atk"], 0, this.props.parentState.pokemonTable, "Atk", "false"),
                effDef: calculateEffStat(event.value, ivSet[this.props.parentState.league]["Default"]["Level"], ivSet[this.props.parentState.league]["Default"]["Def"], 0, this.props.parentState.pokemonTable, "Def", "false"),
                effSta: calculateEffStat(event.value, ivSet[this.props.parentState.league]["Default"]["Level"], ivSet[this.props.parentState.league]["Default"]["Sta"], 0, this.props.parentState.pokemonTable, "Sta"),
    ...
    onIvChange(event) {
        var role = event.target.getAttribute('attr')
        var eff = calculateEffStat(this.state[role].name, this.state[role].Lvl, event.target.value, this.state[role][event.target.name + "Stage"], this.props.parentState.pokemonTable, event.target.name, this.state[role].IsShadow)
        this.setState({
            [role]: {
                ...this.state[role],
    ...
    maximizerSubmit(event) {
        ...
        if (this.state[role]["ivSet"][level] === undefined) {
            var missingLevel = calculateMaximizedStats(this.state[role].name, level, this.props.parentState.pokemonTable)
            switch (action) {
                case "Maximize":
                    varIVS = missingLevel[this.props.parentState.league][stat]
                    break
                default:
                    varIVS = missingLevel[this.props.parentState.league][action]
                    break
            }
            this.setState({
                [role]: {
                    ...
                    "Def": varIVS.Def,
                    "Sta": varIVS.Sta,
                    effAtk: calculateEffStat(this.state[role].name, varIVS.Level, varIVS.Atk, this.state[role].AtkStage, this.props.parentState.pokemonTable, "Atk", this.state[role].IsShadow),
                    effDef: calculateEffStat(this.state[role].name, varIVS.Level, varIVS.Def, this.state[role].DefStage, this.props.parentState.pokemonTable, "Def", this.state[role].IsShadow),
                    effSta: calculateEffStat(this.state[role].name, varIVS.Level, varIVS.Sta, 0, this.props.parentState.pokemonTable, "Sta"),
                    "ivSet": {
        ...
        //otherwise just select from the list
        switch (action) {
            case "Maximize":
                varIVS = this.state[role]["ivSet"][level][this.props.parentState.league][stat]
                break
            default:
                varIVS = this.state[role]["ivSet"][level][this.props.parentState.league][action]
                break
        }
        this.setState({
            [role]: {
    ...
    submitForm = async event => {
        ...
        var reason = ""
        const response = await fetch(((process.env.REACT_APP_LOCALHOST) ? process.env.REACT_APP_LOCALHOST : "") + "/request/single/" + url, {
            method: 'GET',
            headers: {
    ...
    async submitConstructor(constr) {
        ...
        var reason = ""
        const response = await fetch(((process.env.REACT_APP_LOCALHOST) ? process.env.REACT_APP_LOCALHOST : "") + "/request/constructor", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept-Encoding': 'gzip',
